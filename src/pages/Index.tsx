...
import { translate as liveTranslate } from "@/lib/translationClient";
...
const Index = () => {
  ...
  const handleTranslate = async () => {
    ...
    try {
      let result;
      try {
        result = await liveTranslate(sourceText, sourceLang, targetLang, tone);
      } catch (err: any) {
        // If live service fails, fall back to local mock for demo purposes
        toast.warn(`Live translation failed, using offline mock. ${err?.message ?? err}`);
        result = await mockTranslate(sourceText, sourceLang, targetLang, tone);
      }
      setResult(result);
